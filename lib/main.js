...
// Create a global namespace for Mappy so external scripts can access data.
// Currently we're only exposing the map object. If we need to expose more data,
// a better approach is create a public API module e.g. require('./publicAPI')
if (config.map.exposeMapObject === true) {
  window.MAPPY = window.MAPPY || {};

  window.MAPPY.map = map;
  window.MAPPY.leaflet = require('../vendor/leaflet.js');
  window.MAPPY.key = key;
}
