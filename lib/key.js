...
class Key extends EventEmitter {
  ...
  constructor(config) {
    ...
    // either create a new dom element to hold the key or get the container from the page
    // defined in config 'createDomElement: true || false'
    if(config.createDomElement) {
      this.domElement = dom.createElement('div')
      this.domElement.setAttribute('id', config.domElementId)
    } else {
      this.domElement = dom.getElementById(config.domElementId)
    }

    var templateContent = assetManager.assets.html[config.template]
  ...
  renderItem(item, domElement) {
    var templateContent = assetManager.assets.html[this.config.itemTemplate]
